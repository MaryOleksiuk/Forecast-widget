...
export const useFilter = () => {
    ...
    const applyFilter = () => {
        const options = {
            type: type || null,
            minTemperature: minTemperature !== '' ? Number(minTemperature) : null,
            maxTemperature: maxTemperature !== '' ? Number(maxTemperature) : null,
        };

        dispatch(weatherActions.filterDays(options));
    };

    const isFormBlocked = type === '' && minTemperature === '' && maxTemperature === '';
