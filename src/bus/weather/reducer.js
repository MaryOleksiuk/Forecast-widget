    ...
    switch (action.type) {
        ...
        case types.WEATHER_FILTER_DAYS:
            ...
            const filteredDays = state.days.filter((day) => {
                const isCorrectType = options.type ? options.type === day.type : true;
                const isCorrectMinTemperature = options.minTemperature ? options.minTemperature <= day.temperature : true;
                const isCorrectMaxTemperature = options.maxTemperature ? options.maxTemperature >= day.temperature : true;
